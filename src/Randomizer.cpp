...
std::optional<CubeEntry> chooseEntry(
    ...
    auto valid = [&](CubeEntry const& e) {
        if (!e.enabled || !std::isfinite(e.weight) || e.weight <= 0.0) return false;
        if (e.source == IconSource::Vanilla) return e.vanillaID > 0;
        return !e.moreIconsName.empty();
    };

    std::vector<CubeEntry const*> candidates;
    for (auto const& e : entries)
        if (valid(e)) candidates.push_back(&e);
    ...
    if (avoidRepeat && previous && candidates.size() > 1) {
        ...
        for (auto* e : candidates) {
            if (!sameEntry(*e, *previous)) filtered.push_back(e);
        }
        if (!filtered.empty()) candidates = std::move(filtered);
    }

    double total = 0.0;
    for (auto* e : candidates) total += e->weight;
...
bool applyCube(CubeEntry const& entry) {
    if (entry.source == IconSource::Vanilla) {
        if (entry.vanillaID <= 0) return false;
        GameManager::get()->setPlayerFrame(entry.vanillaID);
        return true;
    }

    if (!moreIconsAvailable()) {
        log::warn("Cannot select '{}': More Icons is not installed.", entry.moreIconsName);
        return false;
    }

    auto* icon = more_icons::getIcon(entry.moreIconsName, more_icons::IconType::Cube);
    if (!icon) {
        log::warn("More Icons cube '{}' was not found.", entry.moreIconsName);
        return false;
    }

    more_icons::setIcon(icon, more_icons::IconType::Cube);
    return true;
}
